...
export default function ArticlePage() {
  ...
  const hubHref=category?.slug ? `/categoria/${category.slug}` : "/articulos";
  ...
  return <div data-cat={article.category} className="bg-white">
    <header className="border-b border-[#E1ECF4] bg-[#F1F8FD]"><div className="mx-auto max-w-[1180px] px-5 py-6 md:px-7 md:py-8">
      <nav aria-label="Ruta de navegación" className="mb-6 flex flex-wrap items-center gap-2 text-[0.73rem] text-[#718196]"><Link to="/" className="hover:text-[#1264A3]">Inicio</Link><span>/</span>{category&&<><Link to={hubHref} className="hover:text-[#1264A3]">{category.name}</Link><span>/</span></>}<span className="font-semibold text-[#123B63]">{article.title}</span></nav>
      <div className="max-w-[900px]"><Link to={hubHref} className="mb-4 inline-flex items-center gap-2 rounded-full bg-white px-3.5 py-1.5 text-[0.72rem] font-bold text-[#1264A3] shadow-sm"><span className="h-1.5 w-1.5 rounded-full bg-[var(--cat-accent)]"/>{category?.name}</Link><h1 className="font-display text-[2rem] leading-[1.1] font-bold tracking-[-0.035em] text-[#123B63] md:text-[3.15rem]">{article.title}</h1><p className="mt-4 max-w-[760px] text-[1rem] leading-6 text-[#52657A] md:text-[1.05rem]">{article.description}</p><div className="mt-5 flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#123B63] text-xs font-bold text-white">{INITIALS}</span><div className="text-[0.78rem] text-[#52657A]"><Link to={`/autor/${AUTHOR.slug}`} className="font-bold text-[#123B63]">{AUTHOR.isBrand?`Redacción de ${SITE.name}`:AUTHOR.name}</Link><div className="mt-0.5">{article.readMinutes} min de lectura · Actualizado {formatDateShort(article.date)}</div></div></div></div>
    </div></header>
    <div className="mx-auto max-w-[1180px] px-5 md:px-7"><div className="py-6"><AdSlot variant="leaderboard"/></div>
      <div className="grid items-start gap-10 pb-16 lg:grid-cols-[1fr_300px] lg:gap-14">
        <article><div className="art-visual mb-8 h-[190px] rounded-3xl md:h-[300px]"><span className="absolute bottom-5 left-5 rounded-lg bg-[#123B63]/80 px-3 py-2 text-[0.72rem] font-semibold text-white">{category?.name} · {formatDate(article.date)}</span></div>{body===null?<div className="space-y-3"><div className="h-4 rounded bg-[#EEF3F7]"/><div className="h-4 w-11/12 rounded bg-[#EEF3F7]"/><div className="h-4 w-10/12 rounded bg-[#EEF3F7]"/></div>:<div className="prose-article max-w-[760px]">{md(firstHalf)}</div>}{secondHalf&&<><div className="my-8 max-w-[760px]"><AdSlot variant="in-article" label="Publicidad"/></div><div className="prose-article max-w-[760px]">{md(secondHalf)}</div></>}<div className="mt-10 max-w-[760px] rounded-2xl border border-[#DCE6EE] bg-[#F7FBFE] p-5 md:p-6"><div className="text-[0.67rem] font-bold tracking-[0.12em] text-[#1264A3] uppercase">SIGUE CON EL TEMA</div><div className="mt-2 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-display text-base font-bold text-[#123B63]">Más guías sobre {category?.short?.toLowerCase() ?? "finanzas"}</h2><p className="mt-1 text-sm leading-5 text-[#52657A]">Continúa desde el hub para encontrar la siguiente guía relacionada.</p></div><Link to={hubHref} className="shrink-0 rounded-xl bg-[#1264A3] px-4 py-2.5 text-center text-sm font-bold text-white hover:bg-[#0E568E]">Ver todas las guías →</Link></div></div><div className="mt-6 flex max-w-[760px] justify-between border-t border-[#DCE6EE] pt-5 text-[0.73rem] text-[#718196]"><span>Publicado el {formatDate(article.date)}</span><Link to={hubHref} className="hover:text-[#1264A3]">{category?.name}</Link></div><div className="max-w-[760px]"><AuthorBox/></div></article>
