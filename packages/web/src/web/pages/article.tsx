  ...
  return <div data-cat={article.category} className="bg-white">
    ...
    <div className="mx-auto max-w-[1180px] px-5 md:px-7"><div className="py-6"><AdSlot variant="leaderboard"/></div>
      <div className="grid items-start gap-10 pb-16 lg:grid-cols-[1fr_300px] lg:gap-14">
        <article><div className="art-visual mb-8 h-[190px] rounded-3xl md:h-[300px]"><span className="absolute bottom-5 left-5 rounded-lg bg-[#123B63]/80 px-3 py-2 text-[0.72rem] font-semibold text-white">{category?.name} · {formatDate(article.date)}</span></div>{body===null?<div className="space-y-3"><div className="h-4 rounded bg-[#EEF3F7]"/><div className="h-4 w-11/12 rounded bg-[#EEF3F7]"/><div className="h-4 w-10/12 rounded bg-[#EEF3F7]"/></div>:<div className="prose-article max-w-[760px]">{md(firstHalf)}</div>}{secondHalf&&<><div className="my-8 max-w-[760px]"><AdSlot variant="in-article" label="Publicidad"/></div><div className="prose-article max-w-[760px]">{md(secondHalf)}</div></>}<div className="mt-10 flex max-w-[760px] justify-between border-t border-[#DCE6EE] pt-5 text-[0.73rem] text-[#718196]"><span>Publicado el {formatDate(article.date)}</span><span>{category?.name}</span></div><div className="max-w-[760px]"><AuthorBox/></div></article>
