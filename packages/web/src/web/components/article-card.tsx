...
export function ArticleCard({ article, featured = false, headingLevel = 3 }: { article: Article; featured?: boolean; headingLevel?: 2 | 3 }) {
  ...
  return (
    <Link to={`/articulo/${article.slug}`} data-cat={article.category} className="group flex flex-col overflow-hidden rounded-2xl border border-border bg-card transition-all duration-200 hover:-translate-y-1 hover:border-foreground/15 hover:shadow-[0_24px_42px_-20px_rgba(16,35,61,.28)]">
      <div className={`art-visual ${featured ? "h-32" : "h-28"}`}>
        <span className="absolute top-3.5 left-3.5 z-10 rounded-full bg-[rgba(10,25,48,.88)] px-2.5 py-1 font-mono text-[0.64rem] font-medium tracking-wide text-white">{category?.name}</span>
